...
// CSS vars from <Avatar/>: --av-skin, --av-skin-shadow, plus the rig ramp
// (--av-kit, --av-shoe, --av-contour, ...). The garment colours used to be
// module constants tuned for the old near-black page; they are theme tokens now.
//
// CONTOUR: the outer shapes carry a stroke in --av-contour with
// paint-order:stroke, so the line grows outward and later fills cover the
// internal seams. It is what lets any skin tone sit on any card — ebony
// measured 1.18:1 on a dark surface and porcelain 1.13:1 on a light one, and
// skin tone is not a value we get to adjust. It also survives downscaling,
// which is what makes a 40px leaderboard avatar readable at all.

const TANK = 'var(--av-kit)';
const TANK_SHADOW = 'var(--av-kit-shadow)';
const SHORTS = 'var(--av-kit-alt)';
const SHORTS_SHADOW = 'var(--av-kit-alt-shadow)';
const SHOE = 'var(--av-shoe)';
const SHOE_SOLE = 'var(--av-shoe-sole)';

// Applied to the silhouette-defining shapes only. Interior detail (shading,
// facial features) must not take it or the face turns into a woodcut.
const OUTLINE = {
  ...
  return (
    <g id="av-base-body">
      ...
      <path d="M81,250 C80,256 78,260 74,263 C71,265 72,268 76,268 L96,268 C99,268 100,265 99,262
               C98,258 97,254 97,250 Z" fill={SHOE}  {...OUTLINE}/>
      ...
      <path d="M103,250 C103,254 102,258 101,262 C100,265 101,268 104,268 L124,268 C128,268 129,265 126,263
               C122,260 120,256 119,250 Z" fill={SHOE}  {...OUTLINE}/>
      ...
      <path
        ...
        fill="var(--av-skin)"
       {...OUTLINE}/>
      {/* Shadow hugs the arm's outer edge (143 at y114, 149 at y136, 155 at
          y159). It used to run out to x150 at y112 — roughly 7px past the arm
          — which nothing revealed until the contour drew where the arm ends. */}
      <path
        d="M138,70 C146,76 149,86 147,98 C144,112 146,134 152,156 L155,159 L146,161 C143,146 138,128 135,112
      ...
      <path d="M157,160 C152,155 143,156 140,162 C137,168 140,175 147,176 C154,177 159,171 158,165 Z"
        fill="var(--av-skin)"  {...OUTLINE}/>
      <path d="M152,158 C155,162 156,169 153,173 C157,171 158,164 156,160 Z"
        fill="var(--av-skin-shadow)" opacity="0.5" />
